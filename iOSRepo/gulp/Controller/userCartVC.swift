...
extension userCartVC : STPPaymentContextDelegate {
    ...
    func paymentContext(_ paymentContext: STPPaymentContext, didCreatePaymentResult paymentResult: STPPaymentResult, completion: @escaping STPPaymentStatusBlock) {
        ...
        Functions.functions().httpsCallable("makeCharge").call(data) { (result, error) in
            ...
            let orderticket = Order(customerId: userservice.user.id, merchantId: truckservice.truck.id, items: shoppingCart.itemsOrdered, timestamp: Timestamp.init(), total: shoppingCart.totalCost, additionalRequests: shoppingCart.additionalRequests ?? "", orderNumber: orderNumberValue )
